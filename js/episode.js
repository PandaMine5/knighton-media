...
import { convertEpText, getMetadataKey } from "./misc.js";
...
function getAirdate() {
	...
	return Math.trunc(
      (today.setHours(0, 0, 0, 0) - airdate) / 
      (1000 * 60 * 60 * 24)
    );
}
        ...
        <div class="main-episode__container">
			...
			<div class="main-episode__button-wrapper mb-2">
			    ...
			    <!--<a id="downloadButton" class="primary-button round ${getMetadataKey(ep.scheme_metadata, 'download') ?? 'hidden'}" 
			       href="${ encodeURI(`https://archive.org/download/knighton-media-nk/${getMetadataKey(ep.scheme_metadata, 'download') || ''}?token=${btoa(crypto.randomUUID())}&session=${crypto.randomUUID()}` )}"
			       target="_blank" 
    ...
    <script type="application/ld+json">
		{
		  ...
		  "thumbnailUrl": ["https://pandamine5.github.io/knighton-media/assets/episode-images/${ep.thumbnail.replace('.avif', '.jpg')}"],
